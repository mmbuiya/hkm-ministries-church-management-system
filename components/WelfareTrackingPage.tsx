...
import { UsersIcon, CurrencyDollarIcon, PlusIcon, EyeIcon, FilterIcon, SearchIcon, ChevronUpIcon, ChevronDownIcon } from './Icons';
...
const WelfareTrackingPage: React.FC<WelfareTrackingPageProps> = ({ setActiveView, setActivePage, transactions, members }) => {
    ...
    const memberWelfareData = useMemo(() => {
        ...
        return Array.from(memberData.entries())
            .map(([memberId, data]) => {
                const member = members.find(m => m.id === memberId);
                return { memberId, name: member?.name, phone: member?.phone, status: member?.status, department: member?.department, ...data };
            })
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm border">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 items-end">
                    <div>
                        <label className="text-xs font-medium text-gray-500 mb-1 block" htmlFor="welfare-month">Month</label>
                        <input id="welfare-month" type="month" value={selectedMonth} onChange={e => setSelectedMonth(e.target.value)} className="w-full border-gray-300 rounded-lg px-3 py-2 border shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 text-sm" />
                    </div>
                    <div className="lg:col-span-2">
                        <label className="text-xs font-medium text-gray-500 mb-1 block" htmlFor="welfare-search">Search Member</label>
                        <div className="relative">
                            <SearchIcon className="h-5 w-5 text-gray-400 absolute top-1/2 left-3 -translate-y-1/2" />
                            <input id="welfare-search" type="text" placeholder="Search by name..." value={searchTerm} onChange={e => { setSearchTerm(e.target.value); setCurrentPage(1); }} className="w-full border-gray-300 rounded-lg pl-10 pr-3 py-2 border shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 text-sm" />
                        </div>
                    </div>
                    <div>
                        <label className="text-xs font-medium text-gray-500 mb-1 block" htmlFor="welfare-department">Department</label>
                        <select id="welfare-department" value={selectedDepartment} onChange={e => { setSelectedDepartment(e.target.value); setCurrentPage(1); }} className="w-full border-gray-300 rounded-lg px-3 py-2 border shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 text-sm">
                            <option value="All Departments">All Departments</option>
                            {departments.map(d => <option key={d} value={d}>{d}</option>)}
                        </select>
                    </div>
                    <div>
                        <label className="text-xs font-medium text-gray-500 mb-1 block" htmlFor="welfare-status">Status</label>
                        <select id="welfare-status" value={selectedStatus} onChange={e => { setSelectedStatus(e.target.value); setCurrentPage(1); }} className="w-full border-gray-300 rounded-lg px-3 py-2 border shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 text-sm">
                ...
                <div className="overflow-auto max-h-[65vh]">
                    <table className="w-full text-sm text-left text-gray-500 relative">
                        <thead className="text-xs text-gray-700 uppercase bg-gray-50 sticky top-0 z-10 shadow-sm">
                            <tr>
                                <th scope="col" className="px-6 py-3 bg-gray-50 cursor-pointer select-none hover:bg-gray-100" onClick={() => handleSort('name')} aria-sort={sortField === 'name' ? (sortDirection === 'asc' ? 'ascending' : 'descending') : 'none'} tabIndex={0} onKeyDown={e => e.key === 'Enter' && handleSort('name')}>
                                    Member <SortIcon field="name" />
                                </th>
                                <th scope="col" className="px-6 py-3 bg-gray-50 cursor-pointer select-none hover:bg-gray-100" onClick={() => handleSort('total')} aria-sort={sortField === 'total' ? (sortDirection === 'asc' ? 'ascending' : 'descending') : 'none'} tabIndex={0} onKeyDown={e => e.key === 'Enter' && handleSort('total')}>
                                    Total Welfare <SortIcon field="total" />
                                </th>
                                <th scope="col" className="px-6 py-3 bg-gray-50 cursor-pointer select-none hover:bg-gray-100" onClick={() => handleSort('lastDate')} aria-sort={sortField === 'lastDate' ? (sortDirection === 'asc' ? 'ascending' : 'descending') : 'none'} tabIndex={0} onKeyDown={e => e.key === 'Enter' && handleSort('lastDate')}>
                                    Last Payment Date <SortIcon field="lastDate" />
                                </th>
                                <th scope="col" className="px-6 py-3 bg-gray-50 cursor-pointer select-none hover:bg-gray-100" onClick={() => handleSort('status')} aria-sort={sortField === 'status' ? (sortDirection === 'asc' ? 'ascending' : 'descending') : 'none'} tabIndex={0} onKeyDown={e => e.key === 'Enter' && handleSort('status')}>
