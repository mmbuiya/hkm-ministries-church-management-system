        ...
        <div className="space-y-6">
            <div className="p-6 rounded-lg bg-gradient-to-r from-emerald-600 to-green-500 text-white shadow-lg">
                ...
                <div className="mt-6 flex flex-wrap gap-4 items-center bg-black/10 p-3 rounded-lg">
                    <input type="month" value={selectedMonth} onChange={e => setSelectedMonth(e.target.value)} className="bg-transparent border border-white/30 rounded-md px-3 py-1.5" aria-label="Select month" />
            ...
            {transactions.length === 0 ? (
                <div className="bg-white p-8 rounded-lg shadow-sm border text-center" role="alert">
                    <CurrencyDollarIcon className="h-12 w-12 text-gray-300 mx-auto mb-4" />
                    <h3 className="text-lg font-semibold text-gray-700 mb-2">No Financial Data Yet</h3>
                    <p className="text-gray-500 mb-4">Start recording your church's financial transactions to see insights and reports.</p>
                    <button onClick={() => setActivePage('Add Transaction')} className="bg-green-600 text-white font-semibold py-2 px-6 rounded-lg hover:bg-green-700">
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div className={`${modeColors.card} p-6 rounded-lg shadow-sm`}>
                            ...
                            {incomeVsExpensesChartData.some(d => d.Income > 0 || d.Expenses > 0) ? (
                                <ResponsiveContainer width="100%" height={300}>
                                    <BarChart data={incomeVsExpensesChartData} aria-label="Income vs Expenses bar chart">
                        ...
                        <div className={`${modeColors.card} p-6 rounded-lg shadow-sm`}>
                            <div className="flex flex-wrap justify-between items-center gap-2 mb-4">
                                ...
                                <div className="flex items-center gap-2">
                                    <div className={`flex ${modeColors.bgSecondary} p-1 rounded-lg text-sm`} role="radiogroup" aria-label="Category filter mode">
                                        <button onClick={() => setCategoryFilterMode('all')} className={`px-3 py-1 rounded-md font-semibold ${categoryFilterMode === 'all' ? `${modeColors.card} shadow` : modeColors.text}`} role="radio" aria-checked={categoryFilterMode === 'all'}>All</button>
                                        <button onClick={() => setCategoryFilterMode('year')} className={`px-3 py-1 rounded-md font-semibold ${categoryFilterMode === 'year' ? `${modeColors.card} shadow` : modeColors.text}`} role="radio" aria-checked={categoryFilterMode === 'year'}>Year</button>
                                        <button onClick={() => setCategoryFilterMode('month')} className={`px-3 py-1 rounded-md font-semibold ${categoryFilterMode === 'month' ? `${modeColors.card} shadow` : modeColors.text}`} role="radio" aria-checked={categoryFilterMode === 'month'}>Month</button>
                                    </div>
                                    {categoryFilterMode === 'year' && (
                                        <select
                                            ...
                                            aria-label="Select year for category breakdown"
                                        >
                                            {availableYears.map(year => (
                                                <option key={year} value={year}>{year}</option>
                                            ))}
                    ...
                    <div className={`${modeColors.card} rounded-lg shadow-sm overflow-hidden`}>
                        <div className={`p-4 border-b ${modeColors.border} flex flex-wrap justify-between items-center gap-2`}>
                            ...
                            <div className="flex items-center gap-2">
                                <div className={`flex ${modeColors.bgSecondary} p-1 rounded-lg text-sm`} role="radiogroup" aria-label="Transaction type filter">
                                    <button onClick={() => setTransactionFilter('All')} className={`px-3 py-1 rounded-md font-semibold ${transactionFilter === 'All' ? `${modeColors.card} shadow` : modeColors.text}`} role="radio" aria-checked={transactionFilter === 'All'}>All</button>
                                    <button onClick={() => setTransactionFilter('Income')} className={`px-3 py-1 rounded-md font-semibold ${transactionFilter === 'Income' ? `${modeColors.card} shadow` : modeColors.text}`} role="radio" aria-checked={transactionFilter === 'Income'}>Income</button>
                                    <button onClick={() => setTransactionFilter('Expense')} className={`px-3 py-1 rounded-md font-semibold ${transactionFilter === 'Expense' ? `${modeColors.card} shadow` : modeColors.text}`} role="radio" aria-checked={transactionFilter === 'Expense'}>Expense</button>
                                </div>
                                <a href="#" onClick={(e) => { e.preventDefault(); setActiveView('Transactions'); }} className="text-sm text-green-600 hover:underline">View All ({recentTransactionsCount > 5 ? `${transactions.length} total` : ''})</a>
                            </div>
                        </div>
                        <div className="overflow-x-auto">
                            <table className={`w-full text-sm text-left ${modeColors.textSecondary}`} role="table" aria-label="Recent transactions">
