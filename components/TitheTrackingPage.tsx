...
import { UsersIcon, CurrencyDollarIcon, PlusIcon, EyeIcon, ChevronUpIcon, ChevronDownIcon } from './Icons';
...
type SortField = 'name' | 'total' | 'lastDate';
...
const TitheTrackingPage: React.FC<TitheTrackingPageProps> = ({ setActiveView, setActivePage, transactions, members, onViewHistory }) => {
    ...
    }, [titheRecords, debouncedSearch, members, sortField, sortDirection]);

    const totalPages = Math.ceil(memberTitheData.length / pageSize);
    const paginatedData = useMemo(() => {
        const start = (currentPage - 1) * pageSize;
        return memberTitheData.slice(start, start + pageSize);
    }, [memberTitheData, currentPage, pageSize]);
            ...
            <div className="bg-white rounded-lg shadow-sm overflow-hidden">
                <div className="p-4 border-b flex justify-between items-center">
                    ...
                    <input
                        ...
                        className="border rounded-md px-3 py-1.5 w-1/3"
                ...
                <div className="overflow-auto max-h-[65vh]">
                        ...
                        <thead className="text-xs text-gray-700 uppercase bg-gray-50 sticky top-0 z-10 shadow-sm">
                            <tr>
                                ...
                                <th scope="col" className="px-6 py-3 bg-gray-50 cursor-pointer select-none hover:bg-gray-100" onClick={() => handleSort('total')} aria-sort={sortField === 'total' ? (sortDirection === 'asc' ? 'ascending' : 'descending') : 'none'} tabIndex={0} onKeyDown={e => e.key === 'Enter' && handleSort('total')}>
                                    Total Tithes <SortIcon field="total" />
                                </th>
                                <th scope="col" className="px-6 py-3 bg-gray-50 cursor-pointer select-none hover:bg-gray-100" onClick={() => handleSort('lastDate')} aria-sort={sortField === 'lastDate' ? (sortDirection === 'asc' ? 'ascending' : 'descending') : 'none'} tabIndex={0} onKeyDown={e => e.key === 'Enter' && handleSort('lastDate')}>
                                    Last Tithe Date <SortIcon field="lastDate" />
                                </th>
                    ...
                    {paginatedData.length === 0 && <p className="text-center p-8 text-gray-500">No tithe records found for the selected month.</p>}
                </div>
                <PaginationControls
                    currentPage={currentPage}
                    totalPages={totalPages}
                    totalItems={memberTitheData.length}
                    pageSize={pageSize}
