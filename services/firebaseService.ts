...
export const fbService = {
    ...
    googleSignIn: async () => {
      try {
        ...
        if (error.code === 'auth/popup-closed-by-user') {
          ...
          throw new Error('Pop-up was blocked. Please allow pop-ups for this site.', { cause: error });
        } else if (error.code === 'auth/operation-not-allowed') {
          throw new Error('Google Sign-In is not enabled. Please contact support.', { cause: error });
        } else if (error.code === 'auth/internal-error') {
          throw new Error('Firebase configuration error. Please refresh and try again.', { cause: error });
        } else {
          throw error;
        }
      }
  ...
  userSessions: {
    ...
    getActiveSessions: async (): Promise<UserSession[]> => {
      const q = query(collection(db, 'userSessions'), where('isActive', '==', true), orderBy('lastActivity', 'desc'));
      const snapshot = await getDocs(q);
      return snapshot.docs.map((doc) => doc.data() as UserSession);
    },
  },
